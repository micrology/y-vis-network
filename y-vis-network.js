 ...
 */
const doc = new Y.Doc();
const wsProvider = new WebsocketProvider('ws://localhost:1234', 'y-vis-network-example6', doc);
...
function draw() {
/* 
draw() is unchanged from the standard example from vis-network, with the exception of the 
statements 'data.clientID = undefined;' (added twice).
 */
	// create a network
...
/* 
activate a button to swith the provider on or off (demonstrates that
